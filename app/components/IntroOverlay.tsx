...
export function IntroOverlay({ open, onClose, onNavigate }: IntroOverlayProps) {
  ...
  return (
    <div
      className="intro-backdrop intro-dialog fixed inset-0 z-[400] flex flex-col items-center justify-center text-center space-y-10 p-6"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    >
      ...
      <h1
        className="intro-title text-4xl md:text-6xl font-semibold tracking-wide mb-8 select-none"
        onClick={(e) => e.stopPropagation()}
      >
        Alex Projects
      </h1>
      <div className="intro-links flex flex-col gap-6 text-2xl md:text-3xl font-light text-white/85" onClick={(e) => e.stopPropagation()}>
        <a
          href="#tian"
          onClick={(e) => {
            e.preventDefault();
        ...
        <a
          ...
          onClick={(e) => {
            e.preventDefault();
            onClose();
            setTimeout(() => onNavigate("gallery"), 50);
          }}
          className="hover:text-white transition-all duration-200"
        >
          ✦ Gallery
